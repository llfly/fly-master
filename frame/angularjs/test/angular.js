...
function setupModuleLoader(window) {
  ...
  var angular = ensure(window, 'angular', Object);//é»˜è®¤ window.angular ä¹Ÿæ˜¯ä¸€ä¸ªç©ºå¯¹è±¡

  // We need to expose `angular.$$minErr` to modules such as `ngResource` that reference it during bootstrap
  angular.$$minErr = angular.$$minErr || minErr;

  return ensure(angular, 'module', function() { //ä¸ºangularä¸Šèµ‹ä¸€ä¸ªmoduleæ–¹æ³•
    /** @type {Object.<string, angular.Module>} */
    var modules = {};//ç¼“å­˜çš„moduleå¯¹è±¡
     ...
     * modules. å…¨å±€ä½ç½®ï¼Œç”¨æ¥åˆ›å»ºï¼Œæ³¨å†Œå’ŒèŽ·å–angularæ¨¡å—çš„ä½ç½®ã€‚
     * All modules (angular core or 3rd party) that should be available to an application must be
     * registered using this mechanism.æ‰€æœ‰çš„æ¨¡å—å¿…é¡»é‡‡ç”¨è¿™ç§æœºåˆ¶
     *
     * When passed two or more arguments, a new module is created.  If passed only one argument, an
     * existing module (the name passed as the first argument to `module`) is retrieved.
     * å½“ä¼ å…¥äºŒä¸ªä»¥ä¸Šå‚æ•°çš„æ—¶å€™ï¼Œä¸€ä¸ªæ¨¡å—åˆ›å»ºï¼Œå¦‚æžœä¼ å…¥ä¸€ä¸ªå‚æ•°ï¼ŒèŽ·å–ä¸€ä¸ªå·²ç»å­˜åœ¨çš„å‚æ•°
     *
     * # Module
     *
     * A module is a collection of services, directives, controllers, filters, and configuration information.
     * `angular.module` is used to configure the {@link auto.$injector $injector}.
     * ä¸€ä¸ªæ¨¡å—æ˜¯ä¸€ä¸ªæœåŠ¡ æŒ‡ä»¤ æŽ§åˆ¶å™¨ è¿‡æ»¤å™¨å’Œé…ç½®ä¿¡æ¯çš„é›†åˆ
     * ```js
     * // Create a new module
     * var myModule = angular.module('myModule', []);åˆ›å»ºä¸€ä¸ªæ–°æ¨¡å—
     *
     * // register a new service
     * myModule.value('appName', 'MyCoolApp');æ³¨æ„ä¸€ä¸ªvalue service
     *
     * // configure existing services inside initialization blocks.åœ¨åˆå§‹äººé‡Œé…ç½®å·²ç»å­˜åœ¨çš„æœåŠ¡
     * myModule.config(['$locationProvider', function($locationProvider) {
     *   // Configure existing providers
     *   $locationProvider.hashPrefix('!');é…ç½®hashå‰ç¼€
     * }]);
     * ```
     *
     * Then you can create an injector and load your modules like this ä½ å¯ä»¥åˆ›å»ºä¸€ä¸ªæ³¨å°„å™¨å¹¶ä¸”åƒè¿™æ ·åŠ è½½æ¨¡å—
     *
     * ```js
     ...
     *
     * @param {!string} name The name of the module to create or retrieve.æ¨¡å—åç§°
     * @param {!Array.<string>=} requires If specified then new module is being created. If
     *        unspecified then the module is being retrieved for further configuration.
     * @param {Function=} configFn Optional configuration function for the module. Same as
     *        {@link angular.Module#config Module#config()}. é…ç½®æŒ‡ä»¤
    ...
    return function module(name, requires, configFn) {
      ...
      assertNotHasOwnProperty(name, 'module');//åå­—å¿…å¡«
      if (requires && modules.hasOwnProperty(name)) {//å¦‚æžœå·²ç»æœ‰äº†æ¸…ç©ºé‡æ–°èµ‹å€¼
        modules[name] = null;
      }
      return ensure(modules, name, function() {
        if (!requires) { //å¿…é¡»æŒ‡å®šä¾èµ–çš„æ¨¡å—
        ...
        /** @type {!Array.<Function>} é…ç½®å—*/
        var configBlocks = [];

        /** @type {!Array.<Function>} è¿è¡Œå—*/
        var runBlocks = [];
        ...
        var moduleInstance = {
           ...
           * loaded. å­˜æ”¾åœ¨å½“å‰æ¨¡å—ä¹‹å‰éœ€è¦è¢«è°ƒç”¨çš„å…¶å®ƒæ¨¡å—
           */
          requires: requires,

          /**
           * @ngdoc property
           * @name angular.Module#name æ¨¡å—åç§°
           * @module ng
           *
           * @description
           * Name of the module.
        ...
        if (configFn) {
          config(configFn);//ç«‹åˆ»æ‰§è¡Œé…ç½®
        }

        return moduleInstance;

        /**
         * @param {string} provider æœåŠ¡
         * @param {string} method æ–¹æ³•
